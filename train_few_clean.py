...
# ========= 设置临时文件目录，避免 /tmp 空间不足 =========
os.environ['TMPDIR'] = '/mnt/ecc5c6c9-7631-4983-9ba0-1ec98729589b/tmp'
os.environ['TEMP'] = os.environ['TMPDIR']
os.environ['TMP'] = os.environ['TMPDIR']
os.makedirs(os.environ['TMPDIR'], exist_ok=True)
print(f"[INFO] 临时文件目录设置为: {os.environ['TMPDIR']}")
# =======================================================
    ...
    for epoch in range(args.epoch):
        ...
        for image in support_loader:
            ...
            with torch.no_grad():  # 构建 memory bank 时不需要梯度
                _, seg_patch_tokens, det_patch_tokens = model(image)
                # 这里不去除 CLS token，保持与后续 few-shot 匹配方式一致（使用 p[0].contiguous()）
