...
def main():
    ...
    3）支持检测和分割使用不同的温度参数
    """
    parser = argparse.ArgumentParser(description='Testing with Dual Temperature Support')  # 命令行参数解析器
    ...
    parser.add_argument('--tag', type=str, default=None,
                        help='Optional tag for checkpoint naming, should match train_zero_dual_temp.py')
    ...
    # 打印温度设置信息
    print(f"[INFO] Temperature settings for testing:")
    ...
    # 用当前任务的文本特征做评估，传入分离的温度参数
    print(f"\n[INFO] Starting evaluation on {args.obj} dataset...")
...
def test(args, seg_model, test_loader, text_features, ball=None, temperature_det=10.0, temperature_seg=20.0):
    ...
    print(f'\n[RESULT] {args.obj} Image-level AUC: {round(img_roc_auc_det, 4)}')

    if CLASS_INDEX[args.obj] > 0:
        seg_roc_auc = roc_auc_score(gt_mask_list.flatten(), segment_scores.flatten())
        print(f'[RESULT] {args.obj} Pixel-level AUC: {round(seg_roc_auc, 4)}')
        print(f'[RESULT] {args.obj} Combined Score (pAUC + AUC): {round(seg_roc_auc + img_roc_auc_det, 4)}')
